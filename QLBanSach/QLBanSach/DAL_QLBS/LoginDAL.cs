...
namespace DAL_QLBS
{
    public class LoginDAL
    {
        ...
        {
            return qlbs.users.Where(u => u.user_name == username && u.password == EncryptMD5(password)).Any()?true:false;
        }

        public string getFullName(string username, string password)
        {
            return qlbs.users.Where(u => u.user_name == username && u.password == EncryptMD5(password)).Select(t => t.full_name).FirstOrDefault();
        }
        public string getRole(string username, string password)
        {
            var role = from u in qlbs.users
                       ...
                       where u.user_name == username && u.password == EncryptMD5(password)
                       select r.name;
